...
![Spec version](https://img.shields.io/badge/spec-1.24.0-blue)
...
| `release-manifest.json` | SHA-256 of the bundle, plus a per-file digest for all 798 artifacts, the source commit, and the command to reproduce the bundle. |
...
git archive --prefix=keel-permit-1.24.0/ --format=tar.gz v1.24.0 | sha256sum
...
```sh
gh attestation verify keel-permit-1.24.0.tar.gz -R keelapi/keel-permit
```
