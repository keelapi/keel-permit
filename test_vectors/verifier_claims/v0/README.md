...
The current public and internal verifier CLIs expose whole-pack PASS/FAIL with reason text rather than structured per-claim verdicts. `expected_current` captures that temporary assertion layer, while the `claims[].expected_verdict` fields are the durable doctrine expectations.
