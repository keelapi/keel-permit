#!/usr/bin/env node
// Minimal executable verifier for the permit.co_signature.v2 golden corpus.
...
export function referenceVerify(vector) {
  ...
  const permitDecision = context?.permit_decision;
  if (!claim || !assertion || !key || !context || !permitDecision) {
    return result("insufficient_evidence", "CO_SIGNATURE_EVIDENCE_MISSING");
  }

  if (permitDecision.claim_name !== "permit.decision.v1" ||
      permitDecision.verdict !== "supported") {
    return result("insufficient_evidence", "CO_SIGNATURE_TARGET_DECISION_UNSUPPORTED");
  }
  if (claim.permit_id !== permitDecision.permit_id ||
      claim.permit_decision_canonical_hash !== permitDecision.binding_canonical_hash) {
    return result("disproved", "CO_SIGNATURE_PERMIT_BINDING_MISMATCH");
  }
  if (claim.payload_type !== "permit.co_signature.v2" || claim.key_id !== key.key_id ||
  ...
  const expectedChallenge = Buffer.from(claim.permit_decision_canonical_hash, "hex");
...
if (import.meta.url === invokedPath) {
  ...
  if (report.failures.length > 0) {
    ...
    console.log(`permit.co_signature.v2: ${report.count} vectors matched expected verdicts and reasons`);
  }
}
